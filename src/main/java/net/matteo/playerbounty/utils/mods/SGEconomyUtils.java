...
import static net.matteo.playerbounty.configs.SGEconomyConfig.*;
...
public class SGEconomyUtils {
    ...
    public static MutableComponent SGEconomyDisplay(Player player) {
        ...
        Style hoverEvent = Style.EMPTY.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal("SG-Economy/Magic Coins balance")));

        String balanceString = String.valueOf(GetValues.sg_economy.get(player.getUUID()));
        MutableComponent balance = Component.literal(balanceString).withStyle(bold).withStyle(italic).withStyle(underlined).withStyle(strikethrough);

        component.append(Display1.get());
        component.append(balance.withColor(Color.get()).withStyle(hoverEvent));
